...
const blankProduct = { name: "", desc: "", category: "", weightValue: "100", weightUnit: "g", price: "", discounted_price: "", inStock: "0", image: null , isActive: true, benefits: "", ingredients: "", tags: "" };
...
const saveProduct = async (event) => {
  ...
  try {
    ...
    formData.append("inStock", Number(form.inStock));
    formData.append("isActive", form.isActive);

    formData.append("benefits", JSON.stringify(csv(form.benefits)));
    formData.append("ingredients", JSON.stringify(csv(form.ingredients)));
    formData.append("tags", JSON.stringify(csv(form.tags)));

    // Only send an actual file
  ...
  return <main className="min-h-screen bg-[#f5faf6] px-4 py-10 sm:px-6"><div className="mx-auto max-w-7xl"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="mt-2 text-gray-500">Live store operations and inventory.</p></div><div className="flex gap-3"><button onClick={load} className="rounded-xl border border-green-700 px-4 py-3 text-green-700"><FiRefreshCw /></button><button onClick={openCreate} className="flex items-center gap-2 rounded-xl bg-green-600 px-5 py-3 font-semibold text-white"><FiPlus />Add product</button></div></div>
